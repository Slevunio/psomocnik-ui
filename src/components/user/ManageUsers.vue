                ...
                <div class="container users">
                    <table class="table table-light">
                        ...
                        <tr v-for="(user, index) in users">
                            <td><input type="checkbox" name="userCheckbox" :value="user.id" v-model="checkedUsers"></td>
                            <td>{{index+1}}</td>
                            <td>{{user.username}}</td>
                            <td>{{user.email}}</td>
                            ...
                            <div v-if="role==='ADMIN'">
                                <td>
                                    <router-link :to="{name: 'editUser', params: {userId: user.id}}" tag="button"
                                                 class="btn btn-secondary" :id="'edit'+user.id">Edytuj
                                    </router-link>
                                </td>
                            </div>
                        </tr>
                                ...
                                <div class="modal-footer">
                                    <div class="col-6 text-center">
                                        <button type="submit" class="btn btn-success" data-dismiss="modal"
                                                v-on:click.capture="deleteUsers()">Ok
                                        </button>
                                    </div>
                                    <div class="col-6 text-center">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            checkAll() {
                $("input[name='userCheckbox']").prop("checked", $("#checkAll").prop('checked'));
